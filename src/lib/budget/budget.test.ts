...
/** Crédito de um padrão material do seed (valor × qtd líquida). */
function pad(m: Material | undefined, qtd: number): CreditSide | null {
  const s = side(m, qtd);
  return s ? materialCredit(s) : null;
}

/**
 * Sub-itens de um kit como o resolvedor os entrega: preço do seed (keyed por
 * BaseMaterial, sobrescrevível) e a qtd GRAVADA na planta (sem herança — essa
 * regra é do resolvedor, testada em features/budget).
 */
function kitSubs(
  kit: Kit,
  comp: Componente,
  ...
  return kit.itens.map((item) => {
    const m = seed.materiais.find((x) => x.id === item.materialId)!;
    const valUn = precos[item.materialId] ?? custo(m);
    const qtd = item.id in qtds ? (qtds[item.id] ?? null) : (comp.kitQtds[item.id] ?? null);
    return { item, qtd, herdada: false, valUn, pending: valUn <= 0 };
  });
}
...
function padraoSide(c: Componente, qtd = c.qtd): CreditSide | null {
...
describe("calcBudgetRow (material)", () => {
  ...
  it("T-M1: linha com colunas padrão do projeto (convenção estendida)", () => {
    const r = mustCalc(calcBudgetRow(side(piso002, 18.4, 15)!, pad(piso001, 18.4), cols));
  ...
  it("T-M1b: crédito usa a quantidade líquida, débito usa a quantidade com RT", () => {
    const semRT = mustCalc(calcBudgetRow(side(piso002, 18.4, 0)!, pad(piso001, 18.4), cols));
    const comRT = mustCalc(calcBudgetRow(side(piso002, 18.4, 15)!, pad(piso001, 18.4), cols));
  ...
  it("T-M1c: sem débito/crédito, custoDeTroca = debitoTotal e crédito = 0", () => {
    const comDC = mustCalc(calcBudgetRow(side(piso002, 18.4, 15)!, pad(piso001, 18.4), cols));
    const semDC = mustCalc(
      calcBudgetRow(side(piso002, 18.4, 15)!, pad(piso001, 18.4), cols, {}, false)
    );
  ...
  it("T-M2: subtotal é uma coluna comum e entra no sumFree", () => {
    ...
    const r = mustCalc(calcBudgetRow(side(piso002, 18.4, 15)!, pad(piso001, 18.4), colsComp));
  ...
  it("T-M3: colunas referenciam colunas anteriores pelo nome normalizado", () => {
    ...
    const r = mustCalc(calcBudgetRow(side(piso002, 18.4, 15)!, pad(piso001, 18.4), colsRef));
  ...
  it("T-M4: override por célula é um valor PLANO da linha, não por unidade", () => {
    const r = mustCalc(
      calcBudgetRow(side(piso002, 18.4, 15)!, pad(piso001, 18.4), cols, { [String(cols[0]!.id)]: "10" })
    );
  ...
  it("T-M5: referência desconhecida vira erro na célula e 0 no escopo", () => {
    ...
    const r = mustCalc(calcBudgetRow(side(piso002, 18.4, 15)!, pad(piso001, 18.4), colsErr));
  ...
  it("T-M7: custo de troca e preço travam em zero (nunca negativos)", () => {
    ...
    const r = mustCalc(calcBudgetRow(side(piso001, 18.4)!, pad(piso002, 18.4), cols));
    ...
    const n = mustCalc(calcBudgetRow(side(piso001, 18.4)!, pad(piso002, 18.4), colsNeg));
  ...
  it("T-M8: padrão sem custo credita zero e o upgrade sai pelo débito", () => {
    const naoEntregue = materialCredit({ valUn: 0, qtd: 18.4, rt: 0, pending: false });
...
describe("calcKitRow (kit)", () => {
  ...
  const lado = (c: Componente) => ({ qtd: c.qtd, rt: c.rt });

  // T-K1 — agrega sub-itens e credita o padrão
  it("T-K1: agrega sub-itens e credita o padrão", () => {
    const r = calcKitRow(kitSubs(bronze, metaisComp), lado(metaisComp), padraoSide(metaisComp), cols);
    expect(r.debitoTotal).toBe(1240); // 170*2 + 520*1 + 280*1 + 100*1 (RT 0)
  ...
  // T-K2 — a RT do kit é perda do upgrade, sub-item a sub-item (como no material)
  it("T-K2: RT do kit incide sobre a qtd líquida de cada sub-item no débito", () => {
    const r = calcKitRow(kitSubs(barcelona, salaPisoT1), lado(salaPisoT1), padraoSide(salaPisoT1), cols);
    // (piso-bcn 208 * 18.40 + sol-bcn 115 * 2 + rt-bcn (pendente, 0) * 1.84) * 1.15
    expect(r.debitoTotal).toBeCloseTo(4665.78, 10);
    expect(r.subItems[0]?.qtdComRT).toBeCloseTo(21.16, 10);
    expect(r.subItems[0]?.lineCredito).toBeCloseTo(208 * 18.4, 10); // crédito SEM RT
    expect(r.qtdComRT).toBeCloseTo(21.16, 10); // quantitativo das fórmulas
    expect(r.creditoTotal).toBeCloseTo(1554.8, 10); // 84.5 * 18.40 (SEM RT)
    expect(r.custoDeTroca).toBeCloseTo(3110.98, 10);
    expect(r.sumFree).toBeCloseTo(1430.899, 4);
    expect(r.total).toBeCloseTo(4541.879, 4); // custoDeTroca + sumFree
    expect(r.custoPendente).toBe(true); // rt-bcn tem custoMat 0
  ...
  it("T-P1: sub-item sem custo base marca a linha como pendente", () => {
    ...
    expect(r.custoPendente).toBe(true);
    expect(r.pending).toBe(true);

    // kit todo precificado → sem pendência
    const ok = calcKitRow(kitSubs(bronze, metaisComp), lado(metaisComp), padraoSide(metaisComp), cols);
    ...
    const rz = calcKitRow(
      kitSubs(bronze, metaisComp, { [primeiro.materialId]: 0 }),
      lado(metaisComp),
      padraoSide(metaisComp),
      cols
    );
    expect(rz.subItems[0]?.pending).toBe(true);
    expect(rz.custoPendente).toBe(true);
  });

  // T-K3 — sub-item sem quantidade não vira kit "de graça" em silêncio
  it("T-K3: sub-item sem quantidade vale 0 e deixa a linha pendente", () => {
    const primeiro = bronze.itens[0]!;
    const r = calcKitRow(
      kitSubs(bronze, metaisComp, {}, { [primeiro.id]: null }),
      lado(metaisComp),
      padraoSide(metaisComp),
      cols
    );
    expect(r.subItems[0]?.line).toBe(0);
    expect(r.qtdPendente).toBe(true);
    expect(r.custoPendente).toBe(false);
    expect(r.pending).toBe(true);
    expect(r.debitoTotal).toBe(900); // 1240 - 170*2

    // 0 GRAVADO é quantidade legítima ("não vai nesta planta"): não é pendência.
    const zero = calcKitRow(
      kitSubs(bronze, metaisComp, {}, { [primeiro.id]: 0 }),
      lado(metaisComp),
      padraoSide(metaisComp),
      cols
    );
...
describe("kitCredit (kit padrão)", () => {
  const bronze = kitByNome("Metais Bronze");
  const metaisComp = seed.tipologias[0]!.ambientes[4]!.componentes.find((c) => c.nome === "Metais")!;

  it("credita Σ valor × qtd LÍQUIDA dos sub-itens e alimenta o upgrade de material", () => {
    const credito = kitCredit(kitSubs(bronze, metaisComp));
    expect(credito).toEqual({ credito: 1240, pending: false });
    // Upgrade material sobre um kit padrão: o crédito é o kit inteiro.
    const r = mustCalc(calcBudgetRow({ valUn: 2000, qtd: 1, rt: 0, pending: false }, credito, cols));
    expect(r.creditoTotal).toBe(1240);
    expect(r.custoDeTroca).toBe(760);
  });

  it("fica pendente com sub-item sem custo ou sem quantidade (crédito parcial)", () => {
    const primeiro = bronze.itens[0]!;
    expect(kitCredit(kitSubs(bronze, metaisComp, {}, { [primeiro.id]: null }))).toEqual({
      credito: 900,
      pending: true,
    });
    expect(kitCredit(kitSubs(bronze, metaisComp, { [primeiro.materialId]: 0 })).pending).toBe(true);
  });
});
